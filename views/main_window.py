...
from PySide6.QtWidgets import (
    ...
    QHBoxLayout, QToolButton  # Добавляем QHBoxLayout и QToolButton
)
...
class MainWindow(QMainWindow):
    ...
    def init_db_list_panel(self):
        ...
        # Контейнер для всего блока списка БД
        ...
        self.db_panel_container.setVisible(False)  # Изначально скрыт
        ...
        self.toggle_button.setChecked(False)  # False = Развернуто (показывает "Скрыть")
        self.toggle_button.clicked.connect(self.toggle_db_list_visibility)

        header_layout.addWidget(self.db_list_label)
        header_layout.addStretch(1)  # Заполняем пространство
    ...
    def display_database_list(self, db_list):
        """Отображает полученный список баз данных в QListWidget."""
        ...
        # Показываем всю панель, когда список загружен
        self.db_panel_container.setVisible(True)
        # Убеждаемся, что список и метка внутри панели тоже видны (если не свернуты)
        self.db_list_widget.setVisible(True)
        self.db_list_label.setVisible(True)
        self.toggle_button.setChecked(False)  # Развернуто
        self.toggle_button.setText("Скрыть")
    ...
    def start_reverse_engineering(self, item):
        ...
        try:
            # Окно загрузки (можно заменить на прогресс-бар в реальном приложении)
            ...
            # 1. Очищаем старую диаграмму и загружаем новую схему
            ...
            # После загрузки схемы можно свернуть список БД автоматически
