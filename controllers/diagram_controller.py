...
class DiagramController:
    def get_or_create_diagram_for_project(self, project_id: int, session: Session = None):
        ...
        if session is None: session = SessionLocal(); should_close_session = True
        try:
            ...
            if not diagram:
                ...
                session.add(diagram); session.commit(); session.refresh(diagram)
            return diagram
        finally:
            if should_close_session: session.close()
    def get_diagram_details(self, diagram_id: int) -> list[DiagramObject]:
        session = SessionLocal()
        try: return session.query(DiagramObject).filter_by(diagram_id=diagram_id).options(joinedload(DiagramObject.table).joinedload(Table.columns)).all()
    ...
    def add_existing_table_to_diagram(self, diagram_id: int, table_id: int, x: int, y: int, session: Session = None):
        ...
        try:
            ...
            if should_close_session: session.close()
    def add_new_table_to_diagram(self, diagram_id: int, project_id: int, table_name: str, x: int, y: int) -> (DiagramObject | None):
        session = SessionLocal()
        try:
            ...
            session.add(new_table); session.commit(); session.refresh(new_table)
            default_col = TableColumn(column_name="id", data_type="integer", table_id=new_table.table_id, is_primary_key=True, is_nullable=False)
            session.add(default_col); session.commit()
            new_diagram_object = self.add_existing_table_to_diagram(diagram_id, new_table.table_id, x, y, session=session)
            session.commit()
            session.refresh(new_diagram_object, attribute_names=['table']); session.refresh(new_diagram_object.table, attribute_names=['columns'])
            return new_diagram_object
        except Exception as e:
    ...
    def get_relationships_for_project(self, project_id: int) -> list[Relationship]:
        session = SessionLocal()
        try: return session.query(Relationship).filter_by(project_id=project_id).options(joinedload(Relationship.relationship_columns)).all()
        finally: session.close()
    def add_relationship(self, project_id: int, start_col_id: int, end_col_id: int, start_port_side: str, end_port_side: str) -> Relationship:
        session = SessionLocal()
        try:
            start_col = session.get(TableColumn, start_col_id); end_col = session.get(TableColumn, end_col_id)
            if not start_col or not end_col: return None
            constraint_name = f"fk_{start_col.table.table_name}_{end_col.table.table_name}"
            new_rel = Relationship(project_id=project_id, start_table_id=start_col.table_id, end_table_id=end_col.table_id, constraint_name=constraint_name)
            rel_col = RelationshipColumn(start_column_id=start_col_id, end_column_id=end_col_id, start_port_side=start_port_side, end_port_side=end_port_side)
            new_rel.relationship_columns.append(rel_col); session.add(new_rel); session.commit(); session.refresh(new_rel)
            return new_rel
        except Exception as e:
            session.rollback(); print(f"Ошибка: {e}"); return None
        finally: session.close()
